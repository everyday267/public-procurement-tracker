...
| 전문(366) | 범위형, **최대 `50억이상`** | ❌ 없음 (≥50억까지만) | 동일 |
| 전기(370) | **누적형**(`100억이상`) | ✅ 있음 (연 14.04조) | +한국전력 |

- **월별이 분류축(C3)** 이라 `합계+월` 이중계상 방지 처리(연간=월 합계만).
- **구간 스킴 구분**: 종합·전문은 범위형(합산), 전기는 누적형(단일 구간=총합).
  `ge_threshold_amount()`가 자동 판별.
- **공공 집합**: `{정부기관, 지방자치단체, 공공단체, 공기업}` (`kosis.PUBLIC_AGENCIES`).
- KOSIS 전문은 100억↑ 구간이 없어 전문 100억↑는 KOSIS로 측정 불가 → 대조는
  종합 기준(우리 값은 전문 포함이라 `ratio≳1`이 정상). 정밀 일치가 아닌
  자릿수·추세 확인용(설계 §4.1).
- **kosis.kr 대용량 호출이 간헐적으로 `objL 누락` 오탐 오류를 냄** → 표 단위
  재시도 + `num_periods=1` 축소 폴백 내장. 종합의 다개년 백필은 후속 과제
  (현재 폴백은 최신 연도 확보).

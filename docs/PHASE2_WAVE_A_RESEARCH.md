...
## Wave B — 발전 5사 (2026-07-04 완료)

한전 빅데이터 API의 companyId 재사용(`src/adapters/kepco_family.py`)으로
별도 조사 없이 구현. **실서비스 검증 완료** (run #31, 2026-06):

| 소스 | 기관 | companyId | 공고 | 공사 |
|---|---|---|---:|---:|
| kowepo | 한국서부발전 | COM02 | 57 | 9 |
| kospo | 한국남부발전 | COM04 | 77 | 16 |
| komipo | 한국중부발전 | COM05 | 83 | 7 |
| koen | 한국남동발전 | COM06 | 92 | 10 |
| ewp | 한국동서발전 | COM08 | 74 | 13 |

- 5개사 합계 383건 / 공사 55건 / **100억↑ 0건** / 미공개 0 (6월 기준 —
  발전사 대형 건설공사는 간헐적, 표본 대조 필요)
- bigdata.kepco.co.kr는 연속 조회 시 커넥션 리셋(레이트리밋) — 간격 5초 +
  백오프 최대 90초로 대응 (run #30 실패 → run #31 성공으로 검증)

### bigdata API 운영 관찰 (2022년 연간 백필, 2026-07-06)

- **404 = 결과 0건**: 조회 결과가 없는 기간·companyId 조합에 404를 반환한다
  (2022-12-30~31 × COM06 실측 — 같은 구간 타사 companyId는 1~2건 정상 응답).
  어댑터가 404를 빈 결과로 처리하도록 수정 (`kepco.py _get`).
- **동시 실행 금지**: 여러 워크플로우 런이 병렬로 이 API를 호출하면 발전사
  순회 중 재시도 5회가 소진돼 실패한다 (Q1 백필에서 koen 3개월 연속 실패 →
  단독·순차 재실행은 전부 무재시도 성공). 다개월 백필은 kepco 계열 소스를
  분리해 순차 실행할 것.
- 분기(90일) 단위 조회 정상 동작 확인 — 연간 백필은 월 12회 대신 분기 4회로
  가능하다. 수집 요약 스텝은 all_joined·contracts CSV를 전문 출력하므로
  아티팩트를 못 받는 환경에서도 로그로 데이터 회수가 가능하다.
